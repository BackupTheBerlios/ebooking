<%--
  ...
  @version $Id: title_list.jsp,v 1.1 2005/10/16 18:27:23 raedler Exp $
    ...
    <form method="post">
      <display:table class="data_table"
        ...
        <display:column class="data_table_icon">
          <a href="edit_title.jspa?id=<c:out value="${titleDto.persistentId}"/>">
            <img src="../images/edit.png" alt="<fmt:message key="edit.country"/>"/>
          </a>
        </display:column>
        <display:column class="data_table_icon">
          <a href="delete_title.jspa?id=<c:out value="${titleDto.persistentId}"/>" onclick="return confirm('Wollen Sie das Land <c:out value="${countryDto.name}"/> <c:out value="${countryDto.key}"/> wirklich l&ouml;schen?');">
      ...
      <table class="data_table_extensible_menu">
        <tr>
          <td>
            <a href="list_title.jspa?pagesize=<c:out value="${pagesize}"/>">Selektion aufheben</a>
          </td>
          <td> | </td>
          <td>Kunden pro Seite:
